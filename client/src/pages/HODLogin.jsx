...
const HODLogin = () => {
  ...
  return (
    <Box minHeight="100vh" display="flex" flexDirection="column" alignItems="center" justifyContent="center" sx={{ background: 'transparent', position: 'relative' }}>
      <AnimatedBackground />
      <IconButton onClick={() => navigate('/')} sx={{ position: 'absolute', top: 24, left: 24, color: 'black', zIndex: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
        <HomeIcon fontSize="large" />
        <span style={{ fontWeight: 600, fontSize: 16 }}>Go to Home</span>
      </IconButton>
      <Box display="flex" flexDirection="row" alignItems="center" justifyContent="center" mb={2} zIndex={1} gap={2}>
        <img src={logo} alt="VCET Logo" style={{ height: 60, marginRight: 16 }} />
        <Typography
          variant="h3"
          fontWeight={700}
          align="center"
      ...
      <Box sx={{ background: 'white', borderRadius: 4, boxShadow: 4, p: 3, minWidth: 340, maxWidth: 400, zIndex: 1 }}>
        <LoginCard
          ...
          subtitle="Department Administration Portal"
          icon={<SupervisorAccountIcon fontSize="large" sx={{ color: colors.accent }} />}
          fields={[
